...
public class NhanVienExcelListener extends AnalysisEventListener<NhanVienAdminRequestDTO> {
    ...
    public NhanVienExcelListener(NhanVienAdminRepository nhanVienAdminRepository,
        ...
        this.maNhanVienInDb = new HashSet<>(nhanVienAdminRepository.findAllMaNhanVien());
    }
    ...
    public void invoke(NhanVienAdminRequestDTO data, AnalysisContext context) {
        ...
        if (data.getUsersId() == null) {
            errors.add("Dòng " + rowIndex + ": UserId không được để trống");
            return;
        } else if (usersAdminRepository.existsById(data.getUsersId())) {
            errors.add("Dòng " + rowIndex + ": UserId không được để trống");
            return;
        }
        ...
        if (maNhanVienInFile.contains(maNhanVien)) {
            errors.add("Dòng " + rowIndex + ": Mã trường '" + maNhanVien + "' bị trùng lặp trong file Excel");
            return;
        }
        ...
        if (maNhanVienInDb.contains(maNhanVien)) {
            errors.add("Dòng " + rowIndex + ": Mã trường '" + maNhanVien + "' đã tồn tại trong cơ sở dữ liệu");
            return;
        }
