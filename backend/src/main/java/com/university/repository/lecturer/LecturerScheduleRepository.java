...
public interface LecturerScheduleRepository extends JpaRepository<Lich, UUID> {
    @Query("SELECT l FROM Lich l JOIN l.lopHocPhan lhp JOIN lhp.dGiangDays gd JOIN gd.nhanVien nv JOIN nv.users u WHERE u.id = :userId AND l.ngayHoc BETWEEN :start AND :end")
    ...
    @Query("SELECT l FROM Lich l JOIN l.lopHocPhan lhp JOIN lhp.dGiangDays gd JOIN gd.nhanVien nv JOIN nv.users u WHERE u.id = :userId")
