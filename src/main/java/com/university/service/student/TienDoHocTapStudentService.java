...
public class TienDoHocTapStudentService {
    ...
    public List<TienDoHocTapMonHocStudentResponse> getDanhSachMonHoc(TienDoHocTapStudentRequestDTO request) {
        LearningProgressAggregate aggregate = buildAggregate(request == null ? new TienDoHocTapStudentRequestDTO() : request);
    ...
    private LearningProgressData loadLearningProgressData() {
        ...
        List<ChuongTrinhDaoTao> curriculum =
                tienDoHocTapStudentRepository.findCurriculumByNganhId(hocVien.getNganh().getId());
        List<DangKyTinChi> transcript =
                tienDoHocTapStudentRepository.findTranscriptByHocVienId(hocVienId);
    ...
    private ComputedLearningProgress computeLearningProgress(
        ...
        Map<UUID, TienDoMonSnapshot> bestCourseAttempts =
                selectBestCourseAttempts(loadedData.transcript(), loadedData.gradesByDangKyId());
